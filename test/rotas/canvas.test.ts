...
async function createCanvasTeste() {
    console.log("🔹 Criando table de teste...")
    const token = await loginDeTeste();
    const headers = { 'Authorization': `Bearer ${token}` }
    const response = await axios.post(url_base+'/canvas/add/',{
        "tableId":"1",
    },{headers}).catch(function (error) {
        if (error.response) {
            console.log("Error create Canvas: "+error.response.data)
...
describe("Testes de API Canvas",function(){
    const model = "/canvas"
    it('Deve Retornar 201 para a rota Post e um canvas adicionado /canvas/add', async function() {
        this.timeout(20000)
        const token = await loginDeTeste();
        const headers = { 'Authorization': `Bearer ${token}` }
        const response = await axios.post(url_base+model+'/add/',{"tableId":"1"},{headers}).catch(function (error) {
            if (error.response) {
                return error.response
            }})
        expect(response.status).to.equal(201);
    })
    it('Deve Retornar 401 para ao não receber um token válido, rota Post /canvas/add', async function() {
        this.timeout(20000)
        const response = await axios.post(url_base+model+'/add/',{"tableId":"1"}).catch(function (error) {
            if (error.response) {
                return error.response
            }})
    ...
    it('Deve Retornar 400 para ao receber um token incorreto/invalido, rota Post /canvas/add', async function() {
        this.timeout(20000)
        const token = "TokenIncorreto"
        const headers = { 'Authorization': `Bearer ${token}` }
        const response = await axios.post(url_base+model+'/add/',{"tableId":"1"},{headers:headers}).catch(function (error) {
            if (error.response) {
                return error.response
            }})
        expect(response.status).to.equal(400);
    })
    it('Deve Retornar 200 ao receber um id de canvas existente, rota Get /token/get/1', async function() {
        this.timeout(20000)
        await createCanvasTeste()
    ...
    it('Deve Retornar 404 ao receber um id de canvas inexistente, rota Get /table/get/999', async function() {
        this.timeout(20000)
    ...
    it('Deve Retornar 200 ao receber um id de canvas existente, rota Delete /canvas/delete/1', async function () {
        this.timeout(20000)
        await createCanvasTeste()
        const token = await loginDeTeste();
        const headers = { 'Authorization': `Bearer ${token}` }
    ...
    it('Deve Retornar 404 ao receber um id de canvas inexistente, rota Delete /canvas/delete/999', async function () {
        this.timeout(20000)
        await createTableTeste()
        const token = await loginDeTeste();
        const headers = { 'Authorization': `Bearer ${token}` }
    ...
    it('Deve Retornar 200 ao receber um id de canvas existente, rota Update /canvas/update/1', async function () {
        this.timeout(20000)
        await createCanvasTeste();
        const token = await loginDeTeste();
        const headers = { 'Authorization': `Bearer ${token}` }
    ...
    it('Deve Retornar 404 ao receber um id de table inexistente, rota Update /canvas/update/999', async function () {
        this.timeout(20000)
        await createCanvasTeste();
        const token = await loginDeTeste();
        const headers = { 'Authorization': `Bearer ${token}` }
